public class exercicio {
    public static void main(String[] args) {
      //. Escreva um programa que, com base em uma temperatura em graus celsius, a converta e exiba em
        // Kelvin (K), Réaumur (Re), Rankine (Ra) e Fahrenheit (F),
        // seguindo as fórmulas: F = C * 1.8 + 32; K = C + 273.15; Re = C * 0.8; Ra = C * 1.8 + 32 + 459.67
       double tempC = 30.1;
       double resultF = tempC * 1.8 + 32;
       double resultK = tempC + 273.15;
       double resultRe = tempC * 0.8;
       double resultRa = tempC * 0.8 + 32 + 459.67;


        System.out.println("Temperatura C°: "+tempC+"\nTemperatura F°: "+resultF+"\nTemperatura K°: "+resultK+"\nTemperatura REº: "+resultRe+"\nTemperatura RA°: "+resultRa);


    }
}
